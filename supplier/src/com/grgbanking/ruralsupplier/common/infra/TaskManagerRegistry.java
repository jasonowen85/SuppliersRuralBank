...
public class TaskManagerRegistry {
    ...
    public static void waitAll(final Context context, final Runnable done, final int max, final int interval) {
		...
		com.grgbanking.ruralsupplier.common.infra.Handlers.sharedHandler(context).postDelayed(new Runnable() {
			int count;

			@Override
			public void run() {
				if (!idle() && count++ < max) {
					com.grgbanking.ruralsupplier.common.infra.Handlers.sharedHandler(context).postDelayed(this, interval);
				} else {
					done.run();
				}
			}
