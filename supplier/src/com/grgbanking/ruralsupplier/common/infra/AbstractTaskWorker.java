...
public abstract class AbstractTaskWorker {	
	...
	private final Executor getExecutor(Task task) {
		...
		return com.grgbanking.ruralsupplier.common.infra.TaskExecutor.IMMEDIATE_EXECUTOR;
	}
	
	private final Runnable getRunnable(final com.grgbanking.ruralsupplier.common.infra.Task task) {
		return new Runnable() {
			@Override
			public void run() {
				// schedule
