...
public class RecentContactsFragment extends TFragment implements TAdapterDelegate {
    ...
    public interface sendUnReadListener{
        void sendDataUnread(int unReadNum, String content);
    }

    private sendUnReadListener lister;


    public void OnSendUnReadListerner(sendUnReadListener lister){
       this.lister = lister;
    }
    @Override
    ...
    private void notifyDataSetChanged() {
        ...
        emptyHint.setHint("还没有会话，去工单中看看吧!");
    }
    ...
    private void onRecentContactsLoaded() {
        ...
        if (loadedRecents != null) {
            //过滤
//            for (RecentContact recent : loadedRecents){
//                String blackAccount = Preferences.getBlackUserAccount();
//                if(!TextUtils.isEmpty(blackAccount)){
//                    if(!blackAccount.contains(recent.getFromAccount())){
//                        items.add(recent);
//                    }
//                }
//            }
    ...
    private void refreshMessages(boolean unreadChanged) {
        ...
        //发送到通知栏  最近的一个联系人的消息未读数目， 以及消息内容
        if(lister != null) {
            lister.sendDataUnread( items.get(0).getUnreadCount(), items.get(0).getContent());
        }
        notifyDataSetChanged();
...
//                String blackAccount = Preferences.getBlackUserAccount();
//                if(!TextUtils.isEmpty(blackAccount)){
//                    if(!blackAccount.contains(msg.getFromAccount())){
//                        items.add(msg);
//                    }
//                }
                items.add(msg);
